...
let { flag, binanceArr, mexcArr, bybitArr, gateIoArr, coinbaseArr, lbankArr, kucoinArr, okxArr, bitgetArr, huobiArr, poloniexArr } = require('../ExchangesArray')
let { allCoins, mainData } = require('../../Data')

// склеивает allCoins и создаёт mainData
const collectAllCoins = () => {
    allCoins.data = [...binanceArr.data, ...mexcArr.data, ...bybitArr.data, ...gateIoArr.data, ...coinbaseArr.data, ...lbankArr.data, ...kucoinArr.data, ...okxArr.data, ...bitgetArr.data, ...huobiArr.data, ...poloniexArr.data]
    if (flag.value === 1) {
        mainData.data = []
