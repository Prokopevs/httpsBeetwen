...
const checkSpred = (chain) => {
    ...
    if(chain.stablePrices[100] > 0.9) { // при 100 доларов зароботок больше 0.9
        if((Number(chain.profitInUSDT) < 1.5) || (Number(chain.realPercent) < 0.4) || (Number(chain.maxDealInSellEx) < 200)) {   // если профит меньше 1 долларов c учетом коммисии или спред < 0.4
